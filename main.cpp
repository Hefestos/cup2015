...
int main()
{
	...
	// Config Vision
	CvRect ROI = cvRect(304, 236, 0, 0);
	robot canto_area1 = robot(72, 129);
	robot canto_area2 = robot(230, 200);
	robot canto_gol1 = robot(20, 91);
	robot canto_gol2 = robot(257, 221);
	int amin = 18;
	int amax = 173;
	int radius = 20;
	int min_distance = 12;
	int parametro[36] = { 88, 139, 80, 121, 253, 25,
		21, 62, 114, 60, 243, 255,
		111, 68, 101, 165, 222, 255,
	...
	float t;
	float ref = 0.0;
	float f1 = 0.0 , f2 = 0.0 , torque = 0.0, fx_B = 0.0, d;
	float K;
	float refaux;
	...
	while (1)
	{
		...
		//if (Camera.players[0].x != -1)
			//fprintf(xFile, "%f, %i\n", Camera.players[0].x , int(visionF - start));
		//if (Camera.players[0].y != -1)
			//fprintf(yFile, "%f, %i\n", Camera.players[0].y , int(visionF - start));
		//if (Camera.players[0].x != -1)
			//fprintf(thetaFile, "%f, %i\n", Camera.players[0].theta, int(visionF - start));
		//if (Camera.players[0].x != -1)
			//fprintf(gFile, "%f,%f,%f, %i, %f, %f, %f\n", Camera.players[0].x, Camera.players[0].y, Camera.players[0].theta, int(visionF - start), ref , f1, f2);
		
		//if (Camera.players[0].x != -1)    FUNCIONANDO
			//fprintf(gFile, "%f,%f,%f, %i\n", Camera.players[0].x, Camera.players[0].y, Camera.players[0].theta, int(visionF - start));
		// =========================== ESTRATEGIA =========================== //
		...
		commI = clock();
		 ...
		 x = Camera.players[0].x;
		 y = Camera.players[0].y;
