...
void vision::configParam(int *new_cor, int new_area_min, int new_area_max, int new_time, int new_radius)
{
	radius = new_radius;
	time = new_time;
	for(int i=0; i<6; i++)
...
void vision::configParam(int *new_cor, int new_area_min, int new_area_max, int new_time, int new_radius, CvRect imgROI) 
{
	radius = new_radius;
	time = new_time;
	for (int i = 0; i < 6; i++)
...
bool vision::getImage(CvRect selection)
{
	cvReleaseImage(&fram);
...
void vision::ballPrediction(void)
{

	float angle = ball_old.theta;
	float deltaS = distance(ball_old, players[6]);

	if (deltaS > 0.8)
	{
		players[6].stop = false;
		angle = atan2(players[6].y - ball_old.y, players[6].x - ball_old.x);
	}
	else
	{
		players[6].stop = true;
