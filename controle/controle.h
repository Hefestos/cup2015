...
public:
	...
	void pid_dc(robot, robot);              // calcula as velocidades para o carro se locomover
    void pid_angulo(robot);                 // calcula as velocidades para o carro girar

	void seta_velocidade(robot, robot);
	...
	void error_dc_frente(robot);            // calcula o erro para alcancar um ponto passado
	void error_angulo(robot);               // calcula o erro para ficar no angulo passado
	void controla(robot, robot, robot);     // controla o carrinho levando em consideracao a precisao
	void controla(robot, robot);            // controla o carrinho levando em consideracao a precisao
	void controla_dc(robot, robot);         // controla o carrinho levando em consideracao a precisao
	void controle_bibi(robot, robot, robot);
    ...
    robot v;        // guarda as velocidades a serem usadas pelo motor vy eh a direita, vx a esquerda
